...
class block_vxg_orgs_external extends external_api {
        ...
        public static function get_auto_orgs($query) {
            ...
            $sql_like = $DB->sql_like('fullname', ':search', false);
            $params['search'] = "%$query%";
            $where = '(deleted = 0 OR deleted IS null) AND ' . $sql_like;
            
    
            $orgs = $DB->get_records_select('vxg_org', $where, $params, 'fullname', 'id, fullname');
