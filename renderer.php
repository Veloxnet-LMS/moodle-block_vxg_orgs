...
class block_vxg_orgs_renderer extends plugin_renderer_base
{
    ...
    public function orgs_tree($orgs, $blockid, $returnurl)
    {
        ...
        $sorted_orgs = $this->sort_orgs($orgs);
        ...
        $content .= $OUTPUT->render_from_template('block_vxg_orgs/org_autocomplete_block', array());
        $content .= html_writer::end_div();
        $content .= $this->edit_buttons($returnurl, $blockid);
        $content .= '<ul role="tree" id="orgs_tree" class="block_tree list">';
        $content .= '<li class="contains_branch">
                    <p role="treeitem"
                    class="tree_item branch"
                    aria-owns="main_group"
                    aria-expanded="true"
                    data-orgid="0">
                    <span>' . get_string('all_orgs', 'block_vxg_orgs') . '</span>
                    </p>';

        $content .= '<ul role="group" id="main_group" aria-hidden="false">';

        $content .= $this->get_nodes_recursive($sorted_orgs, $blockid);
        $content .= '</ul>';
        $content .= '</li>';
        $content .= '</ul>';
    ...
    protected function edit_buttons($returnurl)
    {
        ...
        if (has_capability('block/vxg_orgs:caneditorgs', context_system::instance())) {
            ...
            $new_btn = html_writer::link($newurl,
            ...
            $edit_btn = html_writer::link($editurl, 
            ...
            $delete_btn = html_writer::link($delurl, html_writer::tag('button', '<span class="icon fa fa-trash"></span>',
        ...
        if (has_capability('block/vxg_orgs:manageorgadmins', context_system::instance())) {
            $addadminurl = new moodle_url('/blocks/vxg_orgs/manage_admins.php', array(
                'returnurl' => $returnurl));
            $addadmin_btn = html_writer::link($addadminurl, html_writer::tag('button', '<span class="icon fa fa-user-plus"></span>',
                array('class' => 'btn btn-secondary', 'style' => 'background-color:#ced4da', 'data-toggle'=> 'tooltip', 'title'=> get_string('org_admins', 'block_vxg_orgs', ''))), array('style' => 'pointer-events:none;', 'id' => 'vxg_org_addadmin_btn'));

            $buttons .= $addadmin_btn;
        }
